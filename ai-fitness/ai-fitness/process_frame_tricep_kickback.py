...
class ProcessFrame:
    def __init__(self, thresholds, flip_frame = False):
        ...
        self.FEEDBACK_ID_MAP = {
                                0: ('STRAIGHTEN YOUR BACK', 215, (0, 153, 255)),
                                1: ('MOVE HAND FORWARD', 170, (255, 80, 80)),
                                2: ('MOVE HAND BACKWARD', 125, (255, 80, 80))
                               }
    ...
    def _get_state(self, knee_angle):
        ...
        if self.thresholds['ELBOW_WRIST_ANGLE']['NORMAL'][0] >= knee_angle >= self.thresholds['ELBOW_WRIST_ANGLE']['NORMAL'][1]:
            knee = 1
        elif self.thresholds['ELBOW_WRIST_ANGLE']['TRANS'][0] >= knee_angle >= self.thresholds['ELBOW_WRIST_ANGLE']['TRANS'][1]:
            knee = 2
        elif self.thresholds['ELBOW_WRIST_ANGLE']['PASS'][0] >= knee_angle >= self.thresholds['ELBOW_WRIST_ANGLE']['PASS'][1]:
            knee = 3
    ...
    def _show_feedback(self, frame, c_frame, dict_maps, curl_higher_disp):


        if curl_higher_disp:
            draw_text(
                    frame, 
                    'CURL YOUR BICEP HIGHER', 
                    pos=(30, 80),
                    text_color=(0, 0, 0),
                    font_scale=0.6,
    ...
    def process(self, frame: np.array, pose):
        ...
        if keypoints.pose_landmarks:
            ...
            # Camera is aligned properly.
                ...
                shldr_hip_angle = find_angle(shldr_coord, np.array([hip_coord[0], 0]), hip_coord)
                cv2.ellipse(frame, hip_coord, (30, 30), 
                            angle = 0, startAngle = -90, endAngle = -90+multiplier*shldr_hip_angle, 
                            color = self.COLORS['white'], thickness = 3, lineType = self.linetype)

                draw_dotted_line(frame, hip_coord, start=hip_coord[1]-30, end=hip_coord[1]+20, line_color=self.COLORS['aqua'])




                wrist_elbow_angle = find_angle(wrist_coord,shldr_coord, ref_pt = elbow_coord)
                cv2.ellipse(frame, elbow_coord, (20, 20), 
                            angle = 0, startAngle = -90 , endAngle = -90  + multiplier*(wrist_elbow_angle), 
                            color = self.COLORS['rose'], thickness = 3,  lineType = self.linetype)
  

                elbow_shldr_angle = 180 - find_angle(elbow_coord, np.array([shldr_coord[0], 0]), shldr_coord)
                if elbow_coord[0] < shldr_coord[0]:
                    cv2.ellipse(frame, shldr_coord, (20, 20), 
                    angle=0, startAngle= 90, endAngle= 90 +  multiplier * elbow_shldr_angle, 
                    color=self.COLORS['rose'], thickness=3, lineType=self.linetype)
                    flag = True
                else:
                ...
                draw_dotted_line(frame, shldr_coord, start=shldr_coord[1]-50, end=shldr_coord[1]+20, line_color=self.COLORS['aqua'])
                ...
                cv2.line(frame,shldr_coord,hip_coord,self.COLORS['aqua'],2,lineType= self.linetype)
                ...
                current_state = self._get_state(int(wrist_elbow_angle))
                ...
                # -------------------------------------- PERFORM FEEDBACK ACTIONS --------------------------------------
                    ...
                    if shldr_hip_angle > self.thresholds['HIP_THRESH'][1] and shldr_hip_angle <self.thresholds['HIP_THRESH'][0]:
                ...
                knee_text_coord_x = elbow_coord[0] + 15
                ankle_text_coord_x = shldr_coord[0] + 10
                ...
                if 's3' in self.state_tracker['state_seq'] or current_state == 's1':
                    self.state_tracker['CURL_HIGHER'] = False

                self.state_tracker['COUNT_FRAMES'][self.state_tracker['DISPLAY_TEXT']]+=1

                frame = self._show_feedback(frame, self.state_tracker['COUNT_FRAMES'], self.FEEDBACK_ID_MAP, self.state_tracker['CURL_HIGHER'])
                ...
                cv2.putText(frame, str(int(shldr_hip_angle)), (hip_text_coord_x, hip_coord[1]), self.font, 0.6, self.COLORS['light_green'], 2, lineType=self.linetype)
                cv2.putText(frame, str(int(wrist_elbow_angle)), (knee_text_coord_x, elbow_coord[1]+10), self.font, 0.6, self.COLORS['light_green'], 2, lineType=self.linetype)
                cv2.putText(frame, str(int(elbow_shldr_angle)), (ankle_text_coord_x, shldr_coord[1]), self.font, 0.6, self.COLORS['light_green'], 2, lineType=self.linetype)
